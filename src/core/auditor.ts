...
import { analyzeWithGhostProto, analyzeWithGhostProtoAgent } from '../analyzers/ai/ghostproto-analyzer';
...
export async function runAudit(
  ...
  if (!options.noAi && apiKey) {
    if (options.agentic) {
      ...
      try {
        const result = await analyzeWithGhostProtoAgent({
        ...
        try {
          categories = await analyzeWithGhostProto(files, info, apiKey, options.model, options.categories);

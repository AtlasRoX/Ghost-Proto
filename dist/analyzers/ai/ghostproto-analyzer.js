...
exports.analyzeWithGhostProto = analyzeWithGhostProto;
exports.analyzeWithGhostProtoAgent = analyzeWithGhostProtoAgent;
const types_1 = require("../../core/types");
const agent_loop_1 = require("./agent-loop");
const MAX_CONTEXT_CHARS = 60000;
const PRIORITY_EXTENSIONS = ['ts', 'js', 'py', 'go', 'rs', 'java', 'rb'];
function buildCodeContext(files, maxChars) {
...
function getPriorityScore(filePath) {
    ...
    let score = 0;
    if (f.includes('index.') || f.includes('main.') || f.includes('app.'))
        score += 10;
    if (f.includes('auth') || f.includes('security') || f.includes('crypto'))
        score += 8;
    if (f.includes('api') || f.includes('route') || f.includes('controller'))
        score += 6;
    if (f.includes('config') || f.includes('setting') || f.includes('env'))
        score += 5;
    if (f.includes('service') || f.includes('util') || f.includes('helper'))
        score += 4;
    if (f.includes('model') || f.includes('schema') || f.includes('database'))
        score += 4;
    if (f.includes('middleware') || f.includes('hook') || f.includes('plugin'))
        score += 3;
...
function buildAuditPrompt(info, codeContext, filterCategories) {
    const depsStr = Object.entries(info.dependencies).slice(0, 30)
        .map(([k, v]) => `${k}@${v}`).join(', ');
    const allCategories = ['security', 'quality', 'performance', 'architecture', 'testing', 'documentation'];
...
async function analyzeWithGhostProto(files, info, apiKey, model, filterCategories) {
    ...
    const allCategoryOrder = ['security', 'quality', 'performance', 'architecture', 'testing', 'documentation'];
    const categoryOrder = filterCategories ?? allCategoryOrder;
    return categoryOrder.map(cat => {
        const data = parsed[cat];
        if (!data) {
            return {
                category: cat,
                score: 50,
...
async function analyzeWithGhostProtoAgent(opts) {
    const ctx = {
    ...
    const prompt = (0, agent_loop_1.buildInitialUserPrompt)(opts.info, opts.files, opts.filterCategories);
    const result = await (0, agent_loop_1.runAgentLoop)(ctx, {
        apiKey: opts.apiKey,
        model: opts.model,
...
//# sourceMappingURL=ghostproto-analyzer.js.map
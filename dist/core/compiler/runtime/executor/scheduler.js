...
const rule_1 = require("./worker/rule");
const analysis_1 = require("./worker/analysis");
class ExecutionScheduler {
    constructor() {
        this.workers = {
            rule: new rule_1.RuleWorker(),
            analysis: new analysis_1.AnalysisWorker()
        };
    }
    async execute(dag, ctx) {
        if (!dag.validate()) {
            ...
            while (attempt <= task.retries && !success) {
                ...
                try {
                    ...
                    await Promise.race([
                        ...
                        new Promise((_, reject) => setTimeout(() => reject(new Error('Task execution timeout exceeded')), task.timeoutMs))
                    ]);
                    // Verification gate

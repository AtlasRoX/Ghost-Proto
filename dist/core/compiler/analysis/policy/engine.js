...
exports.CompliancePolicyEngine = void 0;
const parser_1 = require("./parser");
const report_1 = require("./report");
const engine_1 = require("../suppress/engine");
class CompliancePolicyEngine {
    constructor(policyConfigStr) {
        this.policy = parser_1.PolicyParser.parse(policyConfigStr);
    }
    async evaluate(ctx, globalSuppressions = []) {
        const violations = [];
        ...
        if (this.policy.complexityLimits?.cyclomatic) {
            ...
            for (const func of functions) {
                const cyclomatic = func.metadata.cyclomatic || 0;
                if (cyclomatic > limit) {
                    violations.push({
        ...
        const suppressionEngine = new engine_1.FindingSuppressionEngine(globalSuppressions);
        const { active, suppressed } = suppressionEngine.filterSuppressions(violations);
        // 5. Build compliance report
        return report_1.ComplianceReportBuilder.build(this.policy.name, this.policy.requiredRulePacks, active, suppressed);
    }
}
exports.CompliancePolicyEngine = CompliancePolicyEngine;

...
describe('AgentLogger', () => {
  it('prints a header with model + budget', () => {
    ...
    log.start({ model: '0.3', maxTurns: 25, maxBudgetTokens: 500000 });
    const out = output();
  ...
  it('marks verbose mode in the header when enabled', () => {
    ...
    log.start({ model: '0.3', maxTurns: 10, maxBudgetTokens: 100000 });
  ...
  it('swallows the "GhostProto is reasoning" progress chatter', () => {
    const { stream, output } = makeStream();
    const log = new AgentLogger({ stream, noColor: true });
    log.progress('GhostProto is reasoning (turn 3/25, 4211 tokens used)...');
  ...
  it('prints warnings emitted via progress()', () => {
    const { stream, output } = makeStream();
    const log = new AgentLogger({ stream, noColor: true });
    log.progress('⚠ GhostProto API error on turn 4: rate limit');
